...
for test_num in range(10):
    ...
    drones = place_drones_scipy(users, num_uav, height)
    ...
    reward_eps.append(np.mean(np.sum(reward)))
    energy.append(np.mean(output))

# print(np.mean(np.array(reward_eps)))
print('mean_phm = ' + str(sum(energy)/10))
# np.savetxt(os.getcwd() + '/' + 'reward_rand.csv', np.array(reward_eps), delimiter=',')
# np.savetxt(os.getcwd() + '/' + 'energy_rand.csv', np.array(energy), delimiter=',')