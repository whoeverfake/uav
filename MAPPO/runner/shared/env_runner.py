...
class EnvRunner(Runner):
    ...
    def run(self):
        ...
        episode = -1
        returns_multi_agent = [[] for i in range(self.num_agents)]
        ...
        # for episode in range(episodes):
            ...
            if episode % self.eval_interval == 0 and self.use_eval:
                evaluate_reward, eval_multi_episode_rewards,_ = self.eval(total_num_steps)
                returns.append(evaluate_reward)
                np.savetxt(self.run_dir/'reward.csv',returns,delimiter=',')

                for i in range(self.num_agents):
                    returns_multi_agent[i].append(eval_multi_episode_rewards[i][0])

                np.savetxt(self.run_dir / 'reward_multi.csv', np.array(returns_multi_agent), delimiter=',')

                plt.figure()
                plt.plot(range(len(returns)), np.array(returns))
    ...
    def eval(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            ...
            # eval_episode_rewards = [x * 0.9 for x in eval_episode_rewards]
        ...
        eval_env_infos['eval_episode_rewards'] = np.sum(np.array(eval_episode_rewards), axis=0) - omega * power_levels[-1]
        # np.savetxt(self.run_dir / 'reward_test_multi.csv', eval_episode_rewards.reshape(self.episode_length, self.num_agents), delimiter=',')
        # np.savetxt(self.run_dir / 'reward_test_sum.csv', np.sum(eval_episode_rewards,axis = 2).reshape(100,1),
        #            delimiter=',')
        # 对每个环境做平均
        eval_average_episode_rewards = np.mean(eval_env_infos['eval_episode_rewards'])
        print("eval average episode rewards of environment: " + str(eval_average_episode_rewards))
        self.log_env(eval_env_infos, total_num_steps)

        """画图"""
        # self.eval_envs.render()
        eval_multi_episode_rewards = np.mean(eval_env_infos['eval_episode_rewards'], axis=0)


        return eval_average_episode_rewards, eval_multi_episode_rewards, phm
        # return eval_average_episode_rewards



    def single_eval(self):
        evaluate_reward, eval_multi_episode_rewards,phm = self.eval(self.episode_length)

        # self.eval_envs.render()
        print("evaluate reward: " + str(evaluate_reward))

        return evaluate_reward, eval_multi_episode_rewards, phm
