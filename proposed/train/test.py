...
def parse_args(args, parser, eval_seed = 1):
    ...
    parser.add_argument('--seed', type=int, default=eval_seed, help="Random seed for numpy/torch")
    """定义agent数量"""
...
def test(args):
    ...
    # np.savetxt(run_dir / 're_vec.csv', np.array(re_vec), delimiter=',') # 奖励reward，取消注释可以保存到本地
    # np.savetxt(run_dir / 'phm_vec.csv', np.array(phm_vec), delimiter=',') # 能效\phi，取消注释可以保存到本地
