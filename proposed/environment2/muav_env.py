...
from environment2.constant_muav import num_gu,num_uav,num_channel,bandwidth,N0,d_min,rcell_max,height,tau_min,power_levels,phi_max,omega
...
def initialize_drones(rcell_max, M):
    drones = np.zeros((M, 2))
    for i in range(M):
        angle_tmp = 2 * pi * i / M
        while True:
            # pos = np.random.uniform(0, self.rcell_max, 2)
            pos = np.array(
                [int(round(rcell_max / 2 * cos(angle_tmp))), int(round(rcell_max / 2 * sin(angle_tmp)))])
            if all(np.linalg.norm(pos - drones[:i, :], axis=1) >= d_min):
                drones[i, :] = pos
                break
    return drones
...
def calc_channel_gain(loc_uav_list, loc_iot_list, num_channels):
    ...
    for i in range(M):
        for j in range(N):
            ...
            g_nlos[i, j] = 20 * np.log10(distance[i, j]) + 20
